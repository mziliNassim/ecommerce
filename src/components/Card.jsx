...
const Card = ({ product }) => {
  ...
  return (
    <>
      <div>
        <div className="flex h-[100%] flex-col justify-between p-5 hover:rounded bg-white border border-gray-200 shadow dark:bg-gray-800 dark:border-gray-700 hover:bg-transparent hover:scale-105 transition-all">
          <div className="flex relative flex-col h-[100%] justify-between">
            <i
              onClick={() => dispatch(toggleFavorite(product.id))}
              className="bi bi-star-fill absolute right-0 top-0 cursor-pointer text-2xl"
              style={{
                color: favorite.includes(product.id) ? "#FFD700" : "#DDD",
              }}
            ></i>

            <Link className="" to={`/product/${product.id}`}>
              <img className="rounded mb-3" src={product.picture} alt="" />

              <h5 className="mb-3 text-md uppercase font-semibold tracking-tight text-gray-900 dark:text-white">
                {product.title}
              </h5>

              <p className="mb-3 mt-auto text-sm font-normal text-gray-700 dark:text-gray-400">
                {product.desc}
              </p>
              ...
              <div className="">
                ...
                <button
                  ...
                  style={{
                    ...
                    background: cartIds.includes(product.id)
                      ? "red"
                      : mainColor,
                  }}
